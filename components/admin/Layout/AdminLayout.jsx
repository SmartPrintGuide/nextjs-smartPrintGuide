...
const AdminLayout = ({ children }) => {
    ...
    const fetchNotifications = async () => {
        try {
            ...
            const { data: chats } = await axios.get(`${process.env.NEXT_PUBLIC_}/chats`, config);
            const unreadChats = chats.filter(c => c.unreadCount > 0);

            // Fetch Orders (Recent & Processing)
            const { data: ordersData } = await axios.get(`${process.env.NEXT_PUBLIC_}/orders?limit=10&page=1`, config);
